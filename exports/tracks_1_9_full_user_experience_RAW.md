## AI Ready ? Tracks 1?9 (Full UX text, raw source export)

This file exports the exact authored lesson text and the exact UI screen copy that renders it.
...
### `lib\screens\scenario_choice_screen.dart`
```
...
class _ScenarioChoiceScreenState extends State<ScenarioChoiceScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (!mounted) return;
      context.read<ProgressStore>().ensureLoaded();
    });
  }
  ...
  /// Sequential lock for Track 1–3 lesson flows (Scenario 1 -> unlock Scenario 2 -> unlock Scenario 3).
  /// Other tracks keep their prior behavior (all scenarios open).
  bool get _useSequentialScenarioUnlock =>
      widget.trackIndex <= 8 && widget.lessonIndex <= 4;

  bool _isScenarioUnlocked(
    ProgressStore store,
    String trackTitle,
    int scenarioIndex,
  ) {
    if (!_useSequentialScenarioUnlock) return true;
    if (scenarioIndex <= 0) return true;
    return store.isScenarioCompleted(
      trackTitle,
      widget.lessonIndex,
      scenarioIndex - 1,
    );
  ...
  Widget build(BuildContext context) {
    ...
    final progressStore = context.read<ProgressStore>();
                    ...
                    child: scenarios.isEmpty
                        ...
                        : ListView.separated(
                            ...
                            itemBuilder: (context, index) {
                              ...
                              final unlocked = _isScenarioUnlocked(
                                progressStore,
                                trackDef.title,
                                index,
                              );
...
class _ScenarioScreenState extends State<ScenarioScreen> {
  /// Premium C.O.R.E. lesson copy (Track 1, lessons 1–2).
  bool get _isTrack1StructuredLesson =>
      widget.trackIndex <= 8 && widget.lessonIndex <= 4;
  ...
  void _showVariantBottomSheet(BuildContext context, ResponseVariant v) {
    ...
    return Wrap(
      ...
      children: List<Widget>.generate(variants.length, (i) {
        ...
        return ActionChip(
          ...
          onPressed: () {
            if (kDebugMode) {
              debugPrint('Learn sheet opened: ${v.label} (${s.title})');
            }
            _showVariantBottomSheet(context, v);
          },
  ...
  Widget build(BuildContext context) {
    ...
    final String promptSectionTitle =
        _isTrack1StructuredLesson ? 'Expert Prompt' : 'What to Ask AI';
    final String variantsSectionTitle =
        _isTrack1StructuredLesson ? 'Learn the C.O.R.E.' : 'Adjust the Result';
    final String tryPracticeCta =
        _isTrack1StructuredLesson ? 'Try the Challenge' : 'Try the Task';
    final String practiceUnavailableCta = _isTrack1StructuredLesson
        ? 'Challenge is being prepared'
        : 'Task is being prepared';
    final hasTask = scenarioDef.task != null;
    final output = scenarioDef.output.trim();
    final bottomInset = MediaQuery.of(context).padding.bottom;
          ...
          SafeArea(
            ...
            child: ValueListenableBuilder<int>(
              ...
              builder: (context, sectionIndex, _) {
                ...
                // 2. Prompt (Expert Prompt on Track 1 Lesson 1)
                _SectionCard(
                  key: _promptKey,
                ...
                // 4. Variant chips (Learn the C.O.R.E. on Track 1 Lesson 1)
                if (hasVariants) ...[
                ...
                builder: (context, tick, _) {
                  ...
                  switch (sectionIndex) {
                    ...
                    case 4:
                      ...
                      bubbleText = _isTrack1StructuredLesson
                          ? "When you're ready, tap Try the Challenge to practice this scenario step-by-step."
              ...
              return SizedBox(
                ...
                child: Opacity(
                  ...
                  child: ElevatedButton(
                    ...
                    child: Text(
                      hasTask ? tryPracticeCta : practiceUnavailableCta,
                    ),
                  ),
                ),
              );
...
class _TaskScreenState extends State<TaskScreen> {
  ...
  /// Premium C.O.R.E. flow: Track 1 lessons 1–2 (indices 0–1).
  bool get _isTrack1StructuredLesson =>
      widget.trackIndex == 0 && widget.lessonIndex <= 4;
...
### `lib\screens\track1_lesson1_combined_challenge_result_screen.dart`
```
import 'package:flutter/material.dart';
import '../data/app_catalog.dart';
import 'scenario_screen.dart';
...
### `lib\screens\track1_scenario3_result_screen.dart`
```
import 'package:flutter/material.dart';
import '../data/app_catalog.dart';
import 'track1_lesson1_complete_screen.dart';
...
### `lib\screens\track1_lesson1_complete_screen.dart`
```
import 'package:flutter/material.dart';
import 'package:provider/provider.dart';
import '../services/progress_store.dart';
    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          ...
          const SizedBox(height: 8),
          const _Bullet('apply context'),
          const _Bullet('define objective'),
          const _Bullet('structure requirements'),
          const _Bullet('control output'),
          const SizedBox(height: 24),
          _InsightBlock(
            title: 'Insight',
            body:
...
### `lib\screens\scenario_incomplete_screen.dart`
```
import 'package:flutter/material.dart';
import '../data/app_catalog.dart';
import 'task_screen.dart';
...
class ScenarioIncompleteScreen extends StatelessWidget {
  ...
  bool get _useChallengeLabel => trackIndex <= 8 && lessonIndex <= 4;
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              Text(
                _useChallengeLabel
                    ? 'You skipped the challenge in ${scenarioDef.title}'
      ...
      bottomNavigationBar: SafeArea(
        ...
        child: Padding(
          ...
          child: Column(
            mainAxisSize: MainAxisSize.min,
            children: [
              // Complete Task / Challenge (filled)
              SizedBox(
                width: double.infinity,
                child: ElevatedButton(
