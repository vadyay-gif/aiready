...
class TaskDef {
  ...
  /// Optional override for combined / Scenario 3 result feedback when the learner is fully correct.
  final String? feedbackWhenCorrect;
  /// Shown on result cards when set; otherwise [assembledPromptExample] is used.
  final String? resultPolishedPrompt;
  const TaskDef({
    required this.goal,
    required this.pieces,
    required this.coachingNote,
    required this.assembledPromptExample,
    this.feedbackWhenCorrect,
    this.resultPolishedPrompt,
  });
}
...
class LessonDef {
  ...
  /// Shown above the first scenario’s situation when set (overlap framing).
  final String? skillRefocusLine;
  const LessonDef(
      {required this.title,
      required this.scenarios,
      this.subtitle,
      this.objectives,
      this.skillRefocusLine});
}
...
class TrackDef {
  ...
  /// One-line capability; shown at top of track screen with [whenToUseThisTrack].
  final String? whatThisTrackBuilds;
  /// One-line use case; shown at top of track screen with [whatThisTrackBuilds].
  final String? whenToUseThisTrack;
  const TrackDef(
      {required this.title,
      required this.icon,
      required this.lessons,
      this.isDisabled = false,
      this.whatThisTrackBuilds,
      this.whenToUseThisTrack});
}
...
final List<TrackDef> kTracks = [
  TrackDef(
    ...
    whatThisTrackBuilds:
        'Clear, controlled wording for everyday professional messages.',
    whenToUseThisTrack:
        'Whenever you write or rewrite email, chat, or short updates under pressure.',
  ),
  TrackDef(
    ...
    whatThisTrackBuilds:
        'Structured outputs from messy inputs—plans, timelines, and reusable prompts.',
    whenToUseThisTrack:
        'When your notes, threads, or tasks need shape you can execute on.',
  ),
  TrackDef(
    ...
    whatThisTrackBuilds:
        'Decision-ready summaries and charts that stay honest to the data.',
    whenToUseThisTrack:
        'When you are working from spreadsheets and must not invent numbers.',
  ), // Track 3
  TrackDef(
    title: 'Presentations',
    ...
    whatThisTrackBuilds:
        'Credible slide narratives and exec-ready framing under ambiguity.',
    whenToUseThisTrack:
        'When stakes are high and the deck has to hold up to scrutiny.',
  ), // Track 4
  TrackDef(
    title: 'Decision\nStructuring',
    ...
    whatThisTrackBuilds:
        'Prompts that turn overload into priorities, plans, and repeatable templates.',
    whenToUseThisTrack:
        'When you need clarity on what to do next—not more generic advice.',
  ), // Track 5
  TrackDef(
    ...
    whatThisTrackBuilds:
        'Audience-right drafts—tone, length, and format without drifting meaning.',
    whenToUseThisTrack:
        'When you adapt or repurpose content for different readers or channels.',
  ), // Track 6
  TrackDef(
    ...
    whatThisTrackBuilds:
        'Diagnostic questions that stay grounded in evidence you actually have.',
    whenToUseThisTrack:
        'When problems feel fuzzy and you need AI to analyze, not fantasize.',
  ), // Track 7
  TrackDef(
    ...
    whatThisTrackBuilds:
        'On-brand copy for posts, pages, and emails with clear constraints.',
    whenToUseThisTrack:
        'When you ship marketing or social assets on a deadline.',
  ), // Track 8
  TrackDef(
    ...
    whatThisTrackBuilds:
        'Ideas and options with trade-offs, guardrails, and next steps.',
    whenToUseThisTrack:
        'When you need breadth, scenarios, or strategy without empty brainstorming.',
  ), // Track 9
];
