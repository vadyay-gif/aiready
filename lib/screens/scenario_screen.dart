...
class _ScenarioScreenState extends State<ScenarioScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final skillRefocus = (lessonDef.skillRefocusLine ?? '').trim();
          ...
          SafeArea(
            ...
            child: ValueListenableBuilder<int>(
              ...
              builder: (context, sectionIndex, _) {
                ...
                return ValueListenableBuilder<double>(
                  ...
                  builder: (context, measuredHeight, _) {
                    ...
                    return ListView(
                      ...
                      children: [
                if (skillRefocus.isNotEmpty && widget.scenarioIndex == 0) ...[
                  Text(
                    skillRefocus,
                ...
                if (_isTrack1StructuredLesson && widget.scenarioIndex == 2) ...[
                  Text(
                    'Most users struggle here — this is where the skill clicks.',
                    style: theme.textTheme.bodyLarge?.copyWith(
                      fontWeight: FontWeight.w600,
                      height: 1.35,
                    ),
...
class _SectionCard extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final isResponse = title.toLowerCase().contains("ai's response");
    final isInsight = title.toLowerCase().contains('pro tip') ||
        title.toLowerCase().contains('learn');
    final bgColor = isInsight
        ? AppColors.insightLight
        : (isResponse ? const Color(0xFFFBFCFF) : AppColors.surface);
    final borderColor = isInsight
        ? AppColors.insight.withValues(alpha: 0.28)
        : (isResponse ? AppColors.primary.withValues(alpha: 0.20) : AppColors.border);
    return Container(
